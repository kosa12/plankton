...
- Original plan: `docs/adr-implementation-plans/TypeScript Hooks
...
**Gate**: This test only runs inside the existing `if [[ -n
"${biome_cmd}" ]]` block. If Biome doesn't flag
...
The ADR test #13 ("Pre-commit: format skip") tests `pre-commit run
biome-format` directly, which is outside the hook's scope — it tests
